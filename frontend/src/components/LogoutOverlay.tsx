...
export function LogoutOverlay({ isVisible }: LogoutOverlayProps) {
  ...
  return (
    ...
    >
      ...
      {/* Card principal */}
      <div
        className={`relative flex flex-col items-center gap-6 rounded-3xl bg-card/80 backdrop-blur-xl p-12 shadow-2xl border border-border/50 transition-all duration-700 ${
      ...
      >
        {/* Icono principal con múltiples círculos */}
        <div className="relative">
          ...
          <div className="relative">
            ...
            <div className="absolute inset-0 -m-4 rounded-full border-4 border-transparent border-b-primary/70 border-l-primary animate-spin" style={{ animationDuration: '2s', animationDirection: 'reverse' }} />
          </div>
          ...
          <div className="relative flex h-24 w-24 items-center justify-center rounded-full bg-primary shadow-lg">
            <LogOut className="h-10 w-10 text-primary-foreground drop-shadow-lg" />
          </div>
        ...
        <div className="text-center space-y-3 min-h-[80px] flex flex-col justify-center">
          <h3 className={`text-2xl font-bold text-foreground transition-all duration-500 ${
            ...
            {step === 0 && 'Cerrando sesión...'}
            {step === 1 && 'Guardando datos...'}
            {step === 2 && '¡Hasta pronto!'}
          </h3>
          <p className={`text-sm text-muted-foreground max-w-xs transition-all duration-500 ${
            isVisible ? 'translate-y-0 opacity-100' : 'translate-y-2 opacity-0'
          }`} style={{ transitionDelay: '100ms' }}>
            {step === 0 && 'Finalizando tu sesión de forma segura'}
            {step === 1 && 'Asegurando tu información'}
            {step === 2 && 'Tu sesión ha sido cerrada correctamente'}
          </p>
        </div>
        ...
        <div className="w-64 h-2 bg-muted rounded-full overflow-hidden shadow-inner">
          <div 
            className="h-full bg-primary rounded-full transition-all duration-1000 ease-out shadow-lg relative"
            style={{ 
