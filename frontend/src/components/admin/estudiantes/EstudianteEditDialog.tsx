/**
 * Diálogo para editar un estudiante existente
 */
...
export function EstudianteEditDialog({ open, onOpenChange, estudianteId }: EstudianteEditDialogProps) {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: Partial<CreateEstudianteDTO>) =>
      estudianteService.updateEstudiante(estudianteId!, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['estudiantes'] });
      queryClient.invalidateQueries({ queryKey: ['estudiante', estudianteId] });
      toast.success('Estudiante actualizado exitosamente', { duration: 3000 });
      onOpenChange(false);
    },
    onError: (error: unknown) => {
      const err = error as { response?: { data?: { message?: string } } };
      const errorMessage = err.response?.data?.message || 'Error al actualizar estudiante';
  ...
  const handleSubmit = () => {
    ...
    const data: Partial<CreateEstudianteDTO> = {
      dni: formData.dni,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Modifique la información del estudiante. Los campos marcados con * son obligatorios.
          </DialogDescription>
        </DialogHeader>
