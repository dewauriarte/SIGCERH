...
export class PDFService {
    STORAGE_DIR = path.join(process.cwd(), 'storage', 'certificados');
    async generarPDF(certificadoId, regenerar = false) {
    ...
    async actualizarCertificadoConPDF(certificadoId, urlPdf, hashPdf, urlQr) {
    ...
    async generarHeader(doc, datos, qrBuffer) {
    ...
    generarDatosEstudiante(doc, datos) {
        const margin = doc.page.margins.left;
        let y = 180;
    ...
    async generarTablaNotas(doc, datos) {
        const margin = doc.page.margins.left;
        const pageWidth = doc.page.width;
        doc.fontSize(11).font('Helvetica-Bold');
        ...
        for (const grado of datos.grados) {
            ...
            doc.text(`${grado.grado} - ${grado.nivel} (${grado.anio})`, margin, doc.y, { underline: true });
            doc.moveDown(0.5);
    ...
    generarFooter(doc, datos) {
        ...
        doc.moveDown();
        doc.fontSize(11).font('Helvetica-Bold');
        doc.text(`PROMEDIO GENERAL: ${datos.promedio.toFixed(2)}`, margin, doc.y, { align: 'center' });
        doc.moveDown();
        doc.fontSize(10).font('Helvetica');
        ...
        doc.moveDown(2);
        doc.fontSize(8);
        doc.text(`Emitido el ${datos.fechaEmision.toLocaleDateString('es-PE')} en ${datos.lugarEmision}`, margin, doc.y, { align: 'center' });
        doc.moveDown();
        doc.fontSize(7);
        doc.text('Este certificado puede ser verificado en https://verificar.ugelpuno.gob.pe', margin, doc.y, { align: 'center' });
    }
}
