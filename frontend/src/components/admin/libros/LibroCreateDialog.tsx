/**
 * Dialog para Crear Nuevo Libro de Actas
...
import { libroService, type CreateLibroDTO } from '@/services/libro.service';
import { nivelEducativoService } from '@/services/nivel-educativo.service';

interface LibroCreateDialogProps {
  open: boolean;
...
export function LibroCreateDialog({ open, onOpenChange }: LibroCreateDialogProps) {
  ...
  const createMutation = useMutation({
    mutationFn: libroService.createLibro,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['libros'] });
      toast.success('Libro creado', {
        description: 'El libro de actas ha sido creado exitosamente.',
      });
      handleClose();
    },
    onError: (error: unknown) => {
      ...
      if (err.response?.data?.errors && Array.isArray(err.response.data.errors)) {
        ...
        toast.error('Error al crear libro', {
          description: err.response?.data?.message || 'No se pudo crear el libro. Verifica los datos e intenta nuevamente.',
          duration: 4000,
        });
      }
    },
  ...
  const updateField = (field: keyof CreateLibroDTO, value: unknown) => {
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Crear Nuevo Libro de Actas
          </DialogTitle>
          <DialogDescription>
            Complete la información del libro. Los campos marcados con * son obligatorios.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={createMutation.isPending}>
            ...
            {createMutation.isPending ? 'Creando...' : 'Crear Libro'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
